...
def CBR(in_ch, out_ch, k=3, p=1, groups=1):
    return nn.Sequential(nn.Conv2d(in_ch, out_ch, kernel_size=k, padding=p, groups=groups, bias=False), nn.BatchNorm2d(out_ch), nn.ReLU(inplace=True))
...
class EfficientSelfAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=True, attn_drop=0.0, proj_drop=0.0, sr_ratio=1):
        super().__init__()
...
class SegFormerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4.0, qkv_bias=True, drop=0.0, attn_drop=0.0, drop_path=0.0, sr_ratio=1):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = EfficientSelfAttention(dim=dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio)
        self.drop_path = DropPath(drop_path)
        self.norm2 = nn.LayerNorm(dim)
        self.mlp = MixFFN(in_features=dim, hidden_features=int(dim * mlp_ratio), out_features=dim, drop=drop)
...
class LightSegFormerB0Encoder(nn.Module):
    ...
    def __init__(self, in_channels=12, embed_dims=(32, 64, 160, 256), num_heads=(1, 2, 5, 8), mlp_ratios=(4, 4, 4, 4), depths=(2, 2, 2, 2), sr_ratios=(8, 4, 2, 1), drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.0):
        super().__init__()
        self.embed_dims = list(embed_dims)
        self.patch_embed1 = OverlapPatchEmbed(in_chans=in_channels, embed_dim=embed_dims[0], kernel_size=7, stride=4, padding=3)
        self.patch_embed2 = OverlapPatchEmbed(in_chans=embed_dims[0], embed_dim=embed_dims[1], kernel_size=3, stride=2, padding=1)
        self.patch_embed3 = OverlapPatchEmbed(in_chans=embed_dims[1], embed_dim=embed_dims[2], kernel_size=3, stride=2, padding=1)
        self.patch_embed4 = OverlapPatchEmbed(in_chans=embed_dims[2], embed_dim=embed_dims[3], kernel_size=3, stride=2, padding=1)
        total_blocks = sum(depths)
        dpr = torch.linspace(0, drop_path_rate, total_blocks).tolist()
        cur = 0
        self.block1 = nn.ModuleList([SegFormerBlock(dim=embed_dims[0], num_heads=num_heads[0], mlp_ratio=mlp_ratios[0], qkv_bias=True, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], sr_ratio=sr_ratios[0]) for i in range(depths[0])])
        cur += depths[0]
        self.block2 = nn.ModuleList([SegFormerBlock(dim=embed_dims[1], num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=True, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], sr_ratio=sr_ratios[1]) for i in range(depths[1])])
        cur += depths[1]
        self.block3 = nn.ModuleList([SegFormerBlock(dim=embed_dims[2], num_heads=num_heads[2], mlp_ratio=mlp_ratios[2], qkv_bias=True, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], sr_ratio=sr_ratios[2]) for i in range(depths[2])])
        cur += depths[2]
        self.block4 = nn.ModuleList([SegFormerBlock(dim=embed_dims[3], num_heads=num_heads[3], mlp_ratio=mlp_ratios[3], qkv_bias=True, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], sr_ratio=sr_ratios[3]) for i in range(depths[3])])
...
class LightSegFormerHead(nn.Module):
    ...
    def __init__(self, in_channels=(32, 64, 160, 256), channels=128, num_classes=NUM_CLASSES, dropout_ratio=0.1):
        super().__init__()
...
class LightSegFormerCloud(nn.Module):
    ...
    def __init__(self, in_channels=IN_CHANNELS, num_classes=NUM_CLASSES):
        super().__init__()
        self.backbone = LightSegFormerB0Encoder(in_channels=in_channels)
        self.head = LightSegFormerHead(in_channels=(32, 64, 160, 256), channels=128, num_classes=num_classes, dropout_ratio=0.1)
...
class LightSegFormerRecon(nn.Module):
    ...
    def __init__(self, in_channels=IN_CHANNELS, num_classes=NUM_CLASSES, recon_name='NDI'):
        super().__init__()
        self.recon_name = recon_name
        self.backbone = LightSegFormerB0Encoder(in_channels=in_channels)
        self.head = LightSegFormerHead(in_channels=(32, 64, 160, 256), channels=128, num_classes=num_classes, dropout_ratio=0.1)
        self.recon_pre = nn.Sequential(CBR(128, 128, k=3, p=1), CBR(128, 128, k=3, p=1))
        self.recon_inject = CBR(128 + 32 + 3, 128, k=1, p=0)
        self.recon_final = nn.Sequential(CBR(128, 64, k=3, p=1), CBR(64, 32, k=3, p=1))
        self.recon_out = nn.Sequential(nn.Conv2d(32, 3, kernel_size=1), nn.Tanh())
    ...
    def forward(self, x, ndi_cloudy, return_shapes=False):
        ...
        feats = self.backbone(x)
        early_feat = feats[0]
        head_feat = self.head.forward_features(feats)
        shared_up = F.interpolate(head_feat, size=early_feat.shape[-2:], mode='bilinear', align_corners=False)
        r128 = self.recon_pre(shared_up)
        ndi_cloudy_early = F.interpolate(ndi_cloudy, size=early_feat.shape[-2:], mode='bilinear', align_corners=False)
        recon_concat = torch.cat([r128, early_feat, ndi_cloudy_early], dim=1)
        r = self.recon_inject(recon_concat)
        r_full = F.interpolate(r, size=input_size, mode='bilinear', align_corners=False)
        r_feat32 = self.recon_final(r_full)
        ndi_pred = self.recon_out(r_feat32)
        shared_full = F.interpolate(head_feat, size=input_size, mode='bilinear', align_corners=False)
        seg_feat = self.seg_fuse(torch.cat([shared_full, r_feat32.detach()], dim=1))
        seg_feat = self.seg_dropout(seg_feat)
        seg_logits = self.seg_out(seg_feat)
        if return_shapes:
            return (seg_logits, ndi_pred, {'input': tuple(x.shape), 'f0_early_H4_32': tuple(feats[0].shape), 'f1_H8_64': tuple(feats[1].shape), 'f2_H16_160': tuple(feats[2].shape), 'f3_H32_256': tuple(feats[3].shape), 'head_feat_H4_128': tuple(head_feat.shape), 'shared_up_H4_128': tuple(shared_up.shape), 'r128_after_two_3x3': tuple(r128.shape), 'ndi_cloudy_H4_3': tuple(ndi_cloudy_early.shape), 'concat_128_plus_32_plus_3': tuple(recon_concat.shape), 'r_after_1x1_128': tuple(r.shape), 'r_feat32': tuple(r_feat32.shape), 'ndi_pred': tuple(ndi_pred.shape), 'shared_full': tuple(shared_full.shape), 'seg_feat_256': tuple(seg_feat.shape), 'seg_logits': tuple(seg_logits.shape)})
...
def train_simple_segformer(model, train_loader, val_loader, device, num_epochs=NUM_EPOCHS_STAGE1, lr=LR_STAGE1, save_path=SIMPLE_SAVE_PATH, patience=PATIENCE_STAGE1):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_iou > best_val_iou + 1e-06:
            ...
            best_epoch = epoch + 1
            bad_epochs = 0
            torch.save({'epoch': epoch + 1, 'val_iou': best_val_iou, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'hist': hist_rows, 'model': 'LightSegFormerCloud', 'num_classes': NUM_CLASSES, 'in_channels': IN_CHANNELS, 'seed': BASE_SEED, 'train_ratio': TRAIN_RATIO, 'val_ratio': VAL_RATIO, 'stage': 'simple_segmentation'}, save_path)
...
def train_segformer_reconstruction(model, train_loader, val_loader, device, num_epochs=NUM_EPOCHS_STAGE2, lr=LR_STAGE2, best_path=RECON_BEST_PATH, patience=PATIENCE_STAGE2):
    ...
    for epoch in range(num_epochs):
        ...
        if val_iou > best_val_iou + 1e-06:
            ...
            best_epoch = epoch + 1
            bad_epochs = 0
            torch.save({'epoch': epoch + 1, 'val_iou': best_val_iou, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'hist': hist_rows, 'model': 'LightSegFormerRecon', 'source_simple_checkpoint': str(SIMPLE_SAVE_PATH), 'num_classes': NUM_CLASSES, 'in_channels': IN_CHANNELS, 'w_seg': W_SEG, 'w_rec': W_REC, 'seed': BASE_SEED, 'train_ratio': TRAIN_RATIO, 'val_ratio': VAL_RATIO, 'encoder_backbone_frozen': True, 'frozen_modules': ['backbone'], 'ndi_mode': 'on_the_fly_no_disk_cache'}, best_path)
        ...
        for x, y, indices in tqdm(loader, desc='Stage 1 selected test eval'):
            ...
            for b in range(preds.shape[0]):
                ...
                if SAVE_TEST_PREDICTIONS:
                    pred_path = SIMPLE_PRED_ROOT / percentage_name / 'pred_masks' / f'lightsegformer_b0_simple_{safe_name(percentage_name)}_{safe_name(sample_key)}.tif'
    ...
    with torch.no_grad():
        for x, y, ndi_cloudy, ndi_clean, mask, indices in tqdm(loader, desc='Stage 2 selected test eval'):
            ...
            for b in range(preds.shape[0]):
                ...
                pred_path = ''
                if SAVE_TEST_PREDICTIONS:
                    pred_path = RECON_PRED_ROOT / percentage_name / 'pred_masks' / f'lightsegformer_b0_recon_rec040_{safe_name(percentage_name)}_{safe_name(sample_key)}.tif'
                    save_prediction_mask(pred_path, preds[b], item['label_path'])
                    pred_path = str(pred_path)
                rows.append({'cloud_percentage': percentage_name, 'sample_key': sample_key, 'roi_id': item['roi_id'], 'prediction_path': pred_path, **metrics_from_confusion(cm_sample)})
...
def run_stage1_simple_segformer():
    ...
    model = LightSegFormerCloud(in_channels=IN_CHANNELS, num_classes=NUM_CLASSES)
    total_params, trainable_params = count_parameters(model)
    print('\nModel parameter count:')
    print(f'Total parameters:     {total_params:,}')
    print(f'Trainable parameters: {trainable_params:,}')
    train_simple_segformer(model=model, train_loader=train_loader, val_loader=val_loader, device=device, num_epochs=NUM_EPOCHS_STAGE1, lr=LR_STAGE1, save_path=SIMPLE_SAVE_PATH, patience=PATIENCE_STAGE1)
...
def run_stage2_reconstruction_from_simple():
    ...
    model = LightSegFormerRecon(in_channels=IN_CHANNELS, num_classes=NUM_CLASSES)
    ...
    print(f'Total parameters:     {total_params:,}')
    print(f'Trainable parameters: {trainable_params:,}')
    train_segformer_reconstruction(model=model, train_loader=train_loader, val_loader=val_loader, device=device, num_epochs=NUM_EPOCHS_STAGE2, lr=LR_STAGE2, best_path=RECON_BEST_PATH, patience=PATIENCE_STAGE2)
...
if __name__ == '__main__':
    print('=' * 80)
    print('Two-stage Light SegFormer-B0 training on LuojiaSET-OSFCR')
    print('=' * 80)
    print('Stage 1 checkpoint:', SIMPLE_SAVE_PATH)
    ...
    if RUN_STAGE_1_SIMPLE_SEGFORMER:
        ...
        print('\nSkipping Stage 1 because RUN_STAGE_1_SIMPLE_SEGFORMER = False')
