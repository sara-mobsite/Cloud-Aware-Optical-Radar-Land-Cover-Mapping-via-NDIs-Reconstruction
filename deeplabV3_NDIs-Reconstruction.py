...
SIMPLE_OUT_ROOT = EXT_ROOT / 'luojia_deeplab_segmentation_seed0_patchsplit_70_30'
SIMPLE_CSV_ROOT = SIMPLE_OUT_ROOT / 'csv'
SIMPLE_PRED_ROOT = SIMPLE_OUT_ROOT / 'test_predictions'
SIMPLE_SAVE_PATH = SIMPLE_OUT_ROOT / 'deeplabv3_luojia_seed0_patchsplit_70_30.pth'
RECON_OUT_ROOT = EXT_ROOT / 'luojia_deeplabv3_reconstruction_only_ndi_rec040_on_the_fly_patchsplit_70_30'
RECON_CSV_ROOT = RECON_OUT_ROOT / 'csv'
RECON_PRED_ROOT = RECON_OUT_ROOT / 'test_predictions'
RECON_BEST_PATH = RECON_OUT_ROOT / 'best_deeplabv3_reconstruction_only_ndi_rec040_from_segmentation_freeze_encoder_no_cache.pth'
...
RUN_STAGE_1_SIMPLE_DEEPLABV3 = True
...
def check_paths():
    ...
    print('Simple DeepLabV3 output root:', SIMPLE_OUT_ROOT)
    print('Reconstruction DeepLabV3 output root:', RECON_OUT_ROOT)
...
class LuojiaDeepLabDataset(Dataset):
...
class LuojiaDeepLabMultitaskNDIDataset(Dataset):
...
def CBR(in_ch, out_ch, k=3, p=1):
    return nn.Sequential(nn.Conv2d(in_ch, out_ch, kernel_size=k, padding=p, bias=False), nn.BatchNorm2d(out_ch), nn.ReLU(inplace=True))

class AtrousSpatialPyramidPooling(nn.Module):

    def __init__(self, in_ch, out_ch, rates=(6, 12, 18)):
        super().__init__()
        self.branches = nn.ModuleList([nn.Sequential(nn.Conv2d(in_ch, out_ch, 1, bias=False), nn.BatchNorm2d(out_ch), nn.ReLU(inplace=True))])
        for r in rates:
        ...
        self.project = nn.Sequential(nn.Conv2d(out_ch * (len(rates) + 2), out_ch, 1, bias=False), nn.BatchNorm2d(out_ch), nn.ReLU(inplace=True), nn.Dropout(0.5))

    def forward(self, x):
        h, w = x.shape[-2:]
        feats = [b(x) for b in self.branches]
        gp = self.global_pool(x)
        gp = F.interpolate(gp, size=(h, w), mode='bilinear', align_corners=True)
        feats.append(gp)
        return self.project(torch.cat(feats, dim=1))

class DeepLabV3Cloud(nn.Module):
    ...
    def forward(self, x):
        ...
        x4 = self.layer4(x3)
        f = self.aspp(x4)
        s = F.interpolate(self.seg_pre(f), size=input_size, mode='bilinear', align_corners=True)
        return self.seg_out(s)
...
class MultiTaskDeepLabV3(nn.Module):
    ...
    def forward(self, x, ndi_cloudy):
        ...
        x4 = self.layer4(x3)
        f = self.aspp(x4)
        r = F.interpolate(f, size=l1.shape[-2:], mode='bilinear', align_corners=True)
        r = self.r64(r)
        ndi_low = F.interpolate(ndi_cloudy, size=l1.shape[-2:], mode='bilinear', align_corners=True)
...
def load_simple_deeplab_into_multitask(model, ckpt_path):
    ckpt = torch.load(ckpt_path, map_location='cpu')
    sd = extract_state_dict(ckpt)
    sd = strip_prefix_if_present(sd, 'module.')
    idx_to_name = {'0': 'conv1', '1': 'bn1', '4': 'layer1', '5': 'layer2', '6': 'layer3', '7': 'layer4'}
    model_sd = model.state_dict()
    loadable = {}
    skipped = []
    for k, v in sd.items():
        new_key = k
        if k.startswith('encoder_layers.') or k.startswith('encoder.'):
            parts = k.split('.')
            idx = parts[1]
        ...
        elif k.startswith('classifier.4.'):
            new_key = k.replace('classifier.4.', 'seg_out.')
        if new_key.startswith(('conv1.', 'bn1.', 'layer1.', 'layer2.', 'layer3.', 'layer4.', 'aspp.', 'seg_pre.', 'seg_out.')):
    ...
    print('\nLoaded simple DeepLabV3 checkpoint into multitask DeepLabV3:')
...
def freeze_deeplab_encoder(model):
    for module in [model.conv1, model.bn1, model.layer1, model.layer2, model.layer3, model.layer4]:
        for p in module.parameters():
            p.requires_grad = False
    trainable = sum((p.numel() for p in model.parameters() if p.requires_grad))
    frozen = sum((p.numel() for p in model.parameters() if not p.requires_grad))
    print('\nDeepLabV3 encoder/backbone frozen.')
...
def make_stage1_loaders():
    ...
    train_dataset = LuojiaDeepLabDataset(train_samples)
    val_dataset = LuojiaDeepLabDataset(val_samples)
...
def make_stage2_loaders():
    worker_init_fn = seed_worker_factory(BASE_SEED)
    train_dataset = LuojiaDeepLabMultitaskNDIDataset(train_samples)
    val_dataset = LuojiaDeepLabMultitaskNDIDataset(val_samples)
    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS, worker_init_fn=worker_init_fn, generator=torch.Generator().manual_seed(BASE_SEED), pin_memory=torch.cuda.is_available(), persistent_workers=False, drop_last=True)
    val_loader = DataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS, worker_init_fn=worker_init_fn, generator=torch.Generator().manual_seed(BASE_SEED), pin_memory=torch.cuda.is_available(), persistent_workers=False, drop_last=False)
    return (train_loader, val_loader)

def train_simple_deeplabv3(model, train_loader, val_loader, device, num_epochs=NUM_EPOCHS_STAGE1, lr=LR_STAGE1, save_path=SIMPLE_SAVE_PATH, patience=PATIENCE_STAGE1):
    ...
    for epoch in range(num_epochs):
        ...
        hist_csv = SIMPLE_CSV_ROOT / 'training_history_deeplabv3_simple.csv'
        ...
        if avg_val_iou > best_val_iou + 1e-06:
            ...
            best_epoch = epoch + 1
            bad_epochs = 0
            torch.save({'epoch': epoch + 1, 'val_iou': best_val_iou, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'hist': hist_rows, 'model': 'DeepLabV3Cloud', 'num_classes': NUM_CLASSES, 'in_channels': IN_CHANNELS, 'seed': BASE_SEED, 'train_ratio': TRAIN_RATIO, 'val_ratio': VAL_RATIO, 'stage': 'simple_segmentation'}, save_path)
...
def train_deeplabv3_reconstruction(model, train_loader, val_loader, device, num_epochs=NUM_EPOCHS_STAGE2, lr=LR_STAGE2, best_path=RECON_BEST_PATH, patience=PATIENCE_STAGE2):
    ...
    for epoch in range(num_epochs):
        ...
        hist_rows.append({'epoch': epoch + 1, 'train_iou': train_iou, 'val_iou': val_iou, 'train_seg_loss': train_seg, 'val_seg_loss': val_seg, 'train_rec_loss': train_rec, 'val_rec_loss': val_rec, 'train_total_loss': train_total, 'val_total_loss': val_total, 'w_seg': W_SEG, 'w_rec': W_REC, 'ndi_mode': 'on_the_fly_no_cache'})
        hist_csv = RECON_CSV_ROOT / 'training_history_deeplabv3_recon_ndi_rec040_no_cache.csv'
        with open(hist_csv, 'w', newline='') as f:
            fieldnames = ['epoch', 'train_iou', 'val_iou', 'train_seg_loss', 'val_seg_loss', 'train_rec_loss', 'val_rec_loss', 'train_total_loss', 'val_total_loss', 'w_seg', 'w_rec', 'ndi_mode']
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(hist_rows)
        ...
        if val_iou > best_val_iou + 1e-06:
            ...
            best_epoch = epoch + 1
            bad_epochs = 0
            torch.save({'epoch': epoch + 1, 'val_iou': best_val_iou, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'hist': hist_rows, 'model': 'MultiTaskDeepLabV3', 'source_simple_checkpoint': str(SIMPLE_SAVE_PATH), 'num_classes': NUM_CLASSES, 'in_channels': IN_CHANNELS, 'w_seg': W_SEG, 'w_rec': W_REC, 'seed': BASE_SEED, 'train_ratio': TRAIN_RATIO, 'val_ratio': VAL_RATIO, 'encoder_frozen': True, 'frozen_modules': ['conv1', 'bn1', 'layer1', 'layer2', 'layer3', 'layer4'], 'ndi_mode': 'on_the_fly_no_cache', 'stage': 'deeplabv3_reconstruction_from_simple_segmentation'}, best_path)
...
def evaluate_simple_selected_test(model, device):
    dataset = LuojiaDeepLabDataset(test_samples, return_index=True)
        ...
        for x, y, indices in tqdm(loader, desc='Stage 1 selected test eval'):
            ...
            for b in range(preds.shape[0]):
                ...
                if SAVE_TEST_PREDICTIONS:
                    pred_path = SIMPLE_PRED_ROOT / percentage_name / 'pred_masks' / f'deeplabv3_simple_{safe_name(percentage_name)}_{safe_name(sample_key)}.tif'
    ...
    write_seg_test_csv('deeplabv3_simple', rows, cm_by_percentage, cm_overall, SIMPLE_CSV_ROOT)

def evaluate_recon_selected_test(model, device):
    dataset = LuojiaDeepLabMultitaskNDIDataset(test_samples, return_index=True)
    loader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS, pin_memory=torch.cuda.is_available())
    model.eval()
    cm_overall = np.zeros((NUM_CLASSES, NUM_CLASSES), dtype=np.int64)
    cm_by_percentage = {}
    ...
    with torch.no_grad():
        for x, y, ndi_cloudy, ndi_clean, mask, indices in tqdm(loader, desc='Stage 2 selected test eval'):
            ...
            for b in range(preds.shape[0]):
                ...
                pred_path = ''
                if SAVE_TEST_PREDICTIONS:
                    pred_path = RECON_PRED_ROOT / percentage_name / 'pred_masks' / f'deeplabv3_recon_rec040_{safe_name(percentage_name)}_{safe_name(sample_key)}.tif'
                    save_prediction_mask(pred_path, preds[b], item['label_path'])
                    pred_path = str(pred_path)
                rows.append({'cloud_percentage': percentage_name, 'sample_key': sample_key, 'roi_id': item['roi_id'], 'prediction_path': pred_path, **metrics_from_confusion(cm_sample)})
    write_seg_test_csv('deeplabv3_recon_rec040', rows, cm_by_percentage, cm_overall, RECON_CSV_ROOT)

def run_stage1_simple_deeplabv3():
    print('\n' + '#' * 80)
    print('STAGE 1: SIMPLE DEEPLABV3 SEGMENTATION')
    print('#' * 80)
    set_seed_everywhere(BASE_SEED)
    ...
    model = DeepLabV3Cloud(pretrained, num_classes=NUM_CLASSES)
    total_params, trainable_params = count_parameters(model)
    print('\nModel parameter count:')
    print(f'Total parameters:     {total_params:,}')
    print(f'Trainable parameters: {trainable_params:,}')
    train_simple_deeplabv3(model=model, train_loader=train_loader, val_loader=val_loader, device=device, num_epochs=NUM_EPOCHS_STAGE1, lr=LR_STAGE1, save_path=SIMPLE_SAVE_PATH, patience=PATIENCE_STAGE1)
...
def run_stage2_reconstruction_from_simple():
    print('\n' + '#' * 80)
    print('STAGE 2: DEEPLABV3 + NDI RECONSTRUCTION FROM SIMPLE CHECKPOINT')
    print('#' * 80)
    ...
    print('\nLoading BigEarthNet pretrained encoder...')
    pretrained = BigEarthNetv2_0_ImageClassifier.from_pretrained('BIFOLD-BigEarthNetv2-0/resnet50-all-v0.2.0')
    model = MultiTaskDeepLabV3(pretrained, num_classes=NUM_CLASSES)
    print('\nLoading saved Stage 1 simple DeepLabV3 weights...')
    load_simple_deeplab_into_multitask(model, SIMPLE_SAVE_PATH)
    print('\nFreezing DeepLabV3 encoder for Stage 2...')
    freeze_deeplab_encoder(model)
    total_params, trainable_params = count_parameters(model)
    print('\nModel parameter count after freezing:')
    print(f'Total parameters:     {total_params:,}')
    print(f'Trainable parameters: {trainable_params:,}')
    train_deeplabv3_reconstruction(model=model, train_loader=train_loader, val_loader=val_loader, device=device, num_epochs=NUM_EPOCHS_STAGE2, lr=LR_STAGE2, best_path=RECON_BEST_PATH, patience=PATIENCE_STAGE2)
...
if __name__ == '__main__':
    print('=' * 80)
    print('Two-stage DeepLabV3 training on LuojiaSET-OSFCR')
    print('=' * 80)
    print('Stage 1 checkpoint:', SIMPLE_SAVE_PATH)
    ...
    print('NDI mode: on-the-fly, no disk cache')
    print('=' * 80)
    if RUN_STAGE_1_SIMPLE_DEEPLABV3:
        run_stage1_simple_deeplabv3()
    else:
        print('\nSkipping Stage 1 because RUN_STAGE_1_SIMPLE_DEEPLABV3 = False')
