...
SIMPLE_OUT_ROOT = EXT_ROOT / "luojia_ppliteseg_t1_segmentation_seed6_patchsplit_70_30"
SIMPLE_CSV_ROOT = SIMPLE_OUT_ROOT / "csv"
SIMPLE_PRED_ROOT = SIMPLE_OUT_ROOT / "test_predictions"
SIMPLE_SAVE_PATH = SIMPLE_OUT_ROOT / "ppliteseg_t1_luojia_seed6_patchsplit_70_30.pth"


RECON_OUT_ROOT = EXT_ROOT / "luojia_ppliteseg_t1_multitask_ndi_rec040_from_simple_patchsplit_70_30_no_cache"
RECON_CSV_ROOT = RECON_OUT_ROOT / "csv"
RECON_PRED_ROOT = RECON_OUT_ROOT / "test_predictions"
RECON_BEST_PATH = RECON_OUT_ROOT / "best_ppliteseg_t1_multitask_ndi_rec040_from_simple.pth"
...
# Run switches.
RUN_STAGE_1_SIMPLE_PPLITESEG = True
...
def check_paths():
    ...
    print("Simple PP-LiteSeg output root:", SIMPLE_OUT_ROOT)
    print("Reconstruction PP-LiteSeg output root:", RECON_OUT_ROOT)
...
# ============================================================
# 4) SAMPLE MATCHING FOR LUOJIASET-OSFCR
# ============================================================
...
class LuojiaPPLiteSegDataset(Dataset):
...
class LuojiaPPLiteSegMultitaskNDIDataset(Dataset):
    """
    Stage 2 reconstruction dataset:
      input: S1 + cloudy S2 + cloudy NDI
...
class SimplePyramidPoolingModule(nn.Module):
    def __init__(self, in_channels, out_channels, pool_scales=(1, 2, 4)):
        ...
        branch_channels = out_channels // len(pool_scales)
        ...
        self.out_conv = ConvBNReLU(
            in_channels + branch_channels * len(pool_scales),
            out_channels,
            kernel_size=3,
            stride=1,
            padding=1,
...
class UAFM(nn.Module):
    ...
    def forward(self, low_feat, high_feat):
        ...
        att = self.attention(torch.cat([low, high], dim=1))

        fused = low * att + high * (1.0 - att)
...
def CBR(in_channels, out_channels, k=3, p=1):
...
class PPLiteSegT1(nn.Module):
    def __init__(self, in_channels=12, num_classes=NUM_CLASSES):
        super().__init__()
        ...
        self.stage5 = nn.Sequential(
            LiteBlock(256, 256, stride=2),
            LiteBlock(256, 256, stride=1),
        )

        self.sppm = SimplePyramidPoolingModule(
            in_channels=256,
            out_channels=128,
            pool_scales=(1, 2, 4),
        )
...
class MultiTaskPPLiteSegT1(nn.Module):
    def __init__(self, in_channels=12, num_classes=NUM_CLASSES):
        super().__init__()
        ...
        # Reconstruction head from the uploaded PP-LiteSeg reconstruction code.
    ...
    def forward(self, x, ndi_cloudy):
        ...
        dec_feat, low64 = self.shared_encoder_decoder(x)
        ...
        s = self.seg_fuse(
            torch.cat([s, r_feat.detach()], dim=1)
        )
...
def load_simple_ppliteseg_into_multitask(model, ckpt_path):
    ...
    for k, v in sd.items():
        # Shared encoder/decoder names are identical.
        if k.startswith((
            "stem.",
            "stage2.",
            "stage3.",
        ...
        # Simple segmentation head -> multitask segmentation modules.
        if k.startswith("seg_head.0."):
            remapped[k.replace("seg_head.0.", "seg_pre.")] = v
            continue

        if k.startswith("seg_head.2."):
            remapped[k.replace("seg_head.2.", "seg_out.")] = v
            continue
    ...
    print("\nLoaded simple PP-LiteSeg checkpoint into multitask PP-LiteSeg:")
    print("Checkpoint:", ckpt_path)
    print("Loaded compatible keys:", len(compatible))
    print("Skipped keys:", len(skipped))
    print("Missing keys are expected for reconstruction head and seg_fuse.")
...
def freeze_ppliteseg_encoder(model):
    """
    Similar to your UNet two-stage protocol:
    freeze the encoder only, keep decoder/segmentation/reconstruction heads trainable.
    """
    encoder_modules = [
        model.stem,
        model.stage2,
    ...
    print("\nPP-LiteSeg encoder frozen.")
    print("Frozen modules: stem, stage2, stage3, stage4, stage5")
...
def make_stage1_loaders():
    ...
    train_dataset = LuojiaPPLiteSegDataset(train_samples)
    val_dataset = LuojiaPPLiteSegDataset(val_samples)
...
def make_stage2_loaders():
    ...
    train_dataset = LuojiaPPLiteSegMultitaskNDIDataset(train_samples)
    val_dataset = LuojiaPPLiteSegMultitaskNDIDataset(val_samples)
...
def train_simple_ppliteseg(
    ...
    for epoch in range(num_epochs):
        ...
        hist_csv = SIMPLE_CSV_ROOT / "training_history_ppliteseg_simple.csv"
...
def train_ppliteseg_reconstruction(
    model,
    train_loader,
    val_loader,
    device,
    ...
    for epoch in range(num_epochs):
        ...
        hist_csv = RECON_CSV_ROOT / "training_history_ppliteseg_recon_rec040.csv"
        ...
        if val_iou > best_val_iou + 1e-6:
            ...
            torch.save({
                ...
                "val_ratio": VAL_RATIO,
                "encoder_frozen": True,
                "frozen_modules": ["stem", "stage2", "stage3", "stage4", "stage5"],
...
def evaluate_simple_selected_test(model, device):
    dataset = LuojiaPPLiteSegDataset(test_samples, return_index=True)
            ...
            for b in range(preds.shape[0]):
                ...
                if SAVE_TEST_PREDICTIONS:
                    pred_path = (
                        ...
                        / percentage_name
                        / "pred_masks"
                        / f"ppliteseg_t1_simple_{safe_name(percentage_name)}_{safe_name(sample_key)}.tif"
                    )
    ...
    write_seg_test_csv("ppliteseg_t1_simple", rows, cm_by_percentage, cm_overall, SIMPLE_CSV_ROOT)


def evaluate_recon_selected_test(model, device):
    dataset = LuojiaPPLiteSegMultitaskNDIDataset(test_samples, return_index=True)
            ...
            for b in range(preds.shape[0]):
                ...
                if SAVE_TEST_PREDICTIONS:
                    pred_path = (
                        ...
                        / percentage_name
                        / "pred_masks"
                        / f"ppliteseg_t1_recon_rec040_{safe_name(percentage_name)}_{safe_name(sample_key)}.tif"
                    )
                    save_prediction_mask(pred_path, preds[b], item["label_path"])
                    pred_path = str(pred_path)
    ...
    write_seg_test_csv("ppliteseg_t1_recon_rec040", rows, cm_by_percentage, cm_overall, RECON_CSV_ROOT)




def run_stage1_simple_ppliteseg():
    print("\n" + "#" * 80)
    print("STAGE 1: SIMPLE PP-LITESEG-T1 SEGMENTATION")
    print("#" * 80)
...
def run_stage2_reconstruction_from_simple():
    print("\n" + "#" * 80)
    print("STAGE 2: PP-LITESEG-T1 + NDI RECONSTRUCTION FROM SIMPLE CHECKPOINT")
    print("#" * 80)
    ...
    print("\nLoading saved Stage 1 simple PP-LiteSeg weights...")
    load_simple_ppliteseg_into_multitask(model, SIMPLE_SAVE_PATH)

    print("\nFreezing PP-LiteSeg encoder for Stage 2...")
    freeze_ppliteseg_encoder(model)
    ...
    train_ppliteseg_reconstruction(
        model=model,
        train_loader=train_loader,
        val_loader=val_loader,
        device=device,
...
if __name__ == "__main__":
    print("=" * 80)
    print("Two-stage PP-LiteSeg-T1 training on LuojiaSET-OSFCR")
    print("=" * 80)
    print("Stage 1 simple checkpoint:", SIMPLE_SAVE_PATH)
    print("Stage 2 reconstruction checkpoint:", RECON_BEST_PATH)
    print(f"Fixed reconstruction weight: W_REC={W_REC}, W_SEG={W_SEG}")
    print("NDI mode: on-the-fly, no disk cache")
    print("=" * 80)

    if RUN_STAGE_1_SIMPLE_PPLITESEG:
        run_stage1_simple_ppliteseg()
    else:
        print("\nSkipping Stage 1 because RUN_STAGE_1_SIMPLE_PPLITESEG = False")
